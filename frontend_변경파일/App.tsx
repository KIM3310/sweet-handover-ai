...
const App: React.FC = () => {
  ...
  // 선택된 인덱스 상태
  const [selectedIndexes, setSelectedIndexes] = useState<RagIndex[]>([]);
  ...
  // 인덱스 선택 변경 핸들러
  const handleIndexChange = (indexes: RagIndex[]) => {
    setSelectedIndexes(indexes);
    console.log("📚 선택된 인덱스:", indexes.map(i => i.name));
  };
  ...
  const handleDeleteSession = (sessionId: string) => {
    ...
    console.log("🗑️ 채팅 세션 삭제됨:", sessionId);
  };
      ...
      if (files.length === 0) {
        ...
        try {
          const response = await fetch(
            `${getBackendUrl()}/api/upload/documents`
          );
          if (response.ok) {
